define(function (require) {
    ...
    /**
     * Detaches and renders the child view.
     *
     * Must be bound to a parent view on SmartView#addChild!
     *
     * @param {string}  name
    ...
    var renderCallback = function (name, overrides) {
        var childView = this.getChild(name),
            wildCard = this.mapping['*'],
            el = childView.el;

        if (wildCard) {
            _.indexOf(this.el.querySelector(wildCard).children, el);
        }
    ...
    var SmartView = View.extend({
        ...
        /**
         * Convenience method got chaining when creating a child View.
        ...
        renderChild: function (child, name, overrides, index) {
            ...
            // TODO: if the child is not mapped up (uses *) then use the index provided
            if (selector) {
