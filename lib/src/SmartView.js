(function (factory) {
    ...
    var View = Backbone.View,
        api = ['build', 'detach', 'update', 'render'],
        noOp = function () {};
    ...
    hbs.registerHelper('models', function (options) {
        return options.data.root.map(options.fn);
    });
    ...
    var SmartView = View.extend({
        ...
        constructor: function (options) {
            ...
            this._fulFillContract(options || {});
            // binding model/collection events
            this.modelEvents.forEach(this._startListening, this);
        },

        // ensure that the current instance fulfill it's class' contract
        _fulFillContract: function (options) {
        ...
        getContext: function (overrides) {
            if (this.model) {
                return $.extend({}, this.model.attributes, overrides);
